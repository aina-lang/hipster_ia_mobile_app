  ...
  export default function FreetextScreen() {
    ...
    const handleSend = async () => {
      ...
      try {
        ...
        const systemContext = `Identité: Hipster IA, Rôle: Assistant créatif`;
        ...
        setMessages((prev) => [...prev, { id: (Date.now() + 1).toString(), text: 'Erreur.', sender: 'ai', timestamp: new Date(), isTyping: true }]);
            ...
            >
              {!hasMessages ? (
                <View className="mt-5 items-center">
                  <Text className="mb-2 text-lg text-slate-500">{getGreetingByTime()} {user?.firstName || 'Utilisateur'}</Text>
