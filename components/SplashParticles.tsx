...
const StarParticle = ({ particle }: { particle: Particle }) => {
  ...
  useEffect(() => {
    ...
    animProgress.value = withRepeat(
      withSequence(
        withTiming(1, { duration: particle.duration * 0.25, easing: Easing.cubic }),
        withTiming(0.2, { duration: particle.duration * 0.15, easing: Easing.ease }),
        withTiming(0.7, { duration: particle.duration * 0.3, easing: Easing.sine }),
        withTiming(0.3, { duration: particle.duration * 0.3, easing: Easing.ease }),
      ),
      -1,
      true
    );
