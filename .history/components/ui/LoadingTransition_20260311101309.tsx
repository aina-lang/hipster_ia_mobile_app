...
const TopBar = React.memo(({ isAuthenticated, userName }: {
  ...
  return (
    <LinearGradient
      ...
      style={styles.topBar}
    >
      <Text style={const TopBar = ({ isAuthenticated, userName }) => {
  const insets = useSafeAreaInsets(); // récupère top/bottom/left/right safe area
  const title = isAuthenticated ? `Bienvenue${userName ? `, ${userName}` : ''}` : 'HIPSTER-IA';
  ...
  return (
    <LinearGradient
      ...
      style={{
        ...
        height: 100 + insets.top, // hauteur + encoche
        paddingTop: insets.top,   // espace safe top
