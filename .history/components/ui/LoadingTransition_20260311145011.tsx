...
// Créer une version animée du LinearGradient
...
// Composant bouton animé séparé pour plus de clarté
const AnimatedGradientButton = ({ onPress, children }) => {
  const progress = useSharedValue(0);
  const scale = useSharedValue(1);
  
  useEffect(() => {
    // Animation du gradient en va-et-vient
  ...
  const animatedGradientProps = useAnimatedProps(() => {
    // Déplace la lumière de gauche à droite
    const startX = interpolate(progress.value, [0, 1], [-0.5, 1.5]);
    const endX = interpolate(progress.value, [0, 1], [0.5, 2.5]);
      ...
      <Animated.View style={buttonStyle}>
        <AnimatedLinearGradient
          colors={[
            ...
            '#1a2f4f',    // Bleu foncé
            '#264F8C',    // Bleu moyen
            '#3a7bb5',    // Bleu clair (lumière)
            '#4d8cd4',    // Bleu très clair
            '#264F8C',    // Retour bleu moyen
            '#1a2f4f',    // Bleu foncé
            '#000000'     // Noir
          ]}
    ...
    <Animated.View style={[styles.container, animStyle]}>
      ...
      <View style={styles.row}>
        ...
        <Pressable onPress={() => { onVideoFinish?.(); router.push('/(auth)/login'); }}>
          <Text small style={styles.link}>Se connecter</Text>
        </Pressable>
      </View>
...
const styles = StyleSheet.create({
  ...
  primaryButton: {
    ...
    overflow: 'hidden',
    borderRadius: 14,
    // Ombre néon pour renforcer l'effet
