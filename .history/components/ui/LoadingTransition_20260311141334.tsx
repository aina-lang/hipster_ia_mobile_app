...
const HelpIcon = ({ onPress }) => {
  ...
  const animatedStyle = useAnimatedStyle(() => ({
    borderColor: `rgba(0, 234, 255, ${borderGlow.value})`,
    transform: [{ scale: scale.value }],
    shadowOpacity: borderGlow.value * 0.5,
  }));

  return (
    <Pressable onPress={onPress} style={styles.helpIconContainer}>
      <Animated.View style={[styles.helpIconBorder, animatedStyle]}>
        <FontAwesome5 
          name="question" 
          size={14}
          color="#ffffff"
...
export const LoadingTransition = React.memo(({ onVideoFinish }: LoadingTransitionProps) => {
  ...
  const insets = useSafeAreaInsets();
  const topBarHeight = 60 + insets.top;
  ...
  return (
    <Animated.View exiting={FadeOut.duration(400)} style={StyleSheet.absoluteFill}>
      ...
      <View style={[StyleSheet.absoluteFill, styles.videoContainer, { marginTop: topBarHeight }]}>
...
const styles = StyleSheet.create({
  ...
  videoContainer: {
    // marginTop est maintenant défini dynamiquement
  },
  particleAnchor: {
    position: 'absolute',
    width: 0,
